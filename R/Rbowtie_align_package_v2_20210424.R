...
filterSeqs <- function(fileDir = getwd(),
  ...
  removeDirectory(newDir,recursive = TRUE,mustExist = FALSE)
  dir.create(newDir)
  ...
  foreach::foreach(file = gzFiles,
    ...
    trim5Bases <- max(nchar(ShortRead::sread(fq)))-trimTo
    if(trim5Bases < 0) trim5Bases <- 0
    fq <- FastqCleaner::fixed_filter(fq,trim5 = trim5Bases)
    fq <- FastqCleaner::adapter_filter(fq,Lpattern = leftPattern,
                                       Rpattern = rightPattern,
                                       anchored = FALSE,
                                       with_indels = TRUE)
    fq <- FastqCleaner::length_filter(fq,rm.min = minLen)
    fq <- FastqCleaner::n_filter(fq,rm.N = 0)
    ShortRead::writeFastq(fq,paste(newDir,newName,sep = "/"))
...
matchAlign <- function(seqData = list.files(pattern = "*.fastq.gz$"),
  ...
  aligner = foreach::foreach(i = 1:length(seqData),
                             .combine = cbind,
                             .packages = c("tidyverse","ShortRead","data.table"),
                             .options.snow = opts) %dopar% {
...
btAlign <- function(seqData = list.files(pattern = "*.fastq.gz$"),
  ...
  tidyverse::write_tsv(as.data.frame(tsvFile),tsvFileName)
...
bt2Align <- function(seqData = list.files(pattern = "*.fastq.gz$"),
  ...
  if(numCores > snow::detectCores()) {
    numCores <- snow::detectCores()
  }
  ...
  bt2Out <- foreach::foreach(i = 1:length(seqData),
                             .combine = c,
                             .packages = c("ShortRead","Rbowtie2","tidyverse","Rsamtools","R.utils"),
                             .options.snow = opts) %dopar% {
